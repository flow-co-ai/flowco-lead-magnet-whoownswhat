/* ============================================================
   ...
   Desktop-only script (loaded globally; safe when .desktop-site
   is hidden on mobile — all DOM queries resolve to existing
   off-screen elements, no errors thrown)
   ============================================================ */
...
const clearBtn = document.querySelector(".filter-tag__clear");
if (clearBtn) {
  clearBtn.addEventListener("click", () => { activeFilter = null; applyFilter(); });
}

/* ---------------- Gate form (multi-field) + capture.js integration ---------------- */
...
if (gateForm) {
  gateForm.addEventListener("submit", (e) => {
    e.preventDefault();
    const nameVal    = gateName    ? gateName.value.trim()    : "";
    const companyVal = gateCompany ? gateCompany.value.trim() : "";
    const emailVal   = gateEmail   ? gateEmail.value.trim()   : "";

    const emailOk = /^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(emailVal);

    if (!nameVal || !companyVal || !emailOk) {
      if (gateNote) {
        gateNote.classList.add("gate__error");
        gateNote.querySelector("span").textContent = "Please fill in all fields with a valid email.";
      }
      if (!nameVal && gateName)       { gateName.focus();    return; }
      if (!companyVal && gateCompany) { gateCompany.focus(); return; }
      if (!emailOk && gateEmail)      { gateEmail.focus();   return; }
      return;
    }
    ...
    fetch("/.netlify/functions/capture", {
      ...
      body: JSON.stringify({ name: nameVal, email: emailVal, company: companyVal, medium: "Organic" }),
    }).catch(() => {});

    try { localStorage.setItem("flowco_lead_email", emailVal); } catch (_) {}

    if (gate)         gate.classList.add("unlocked");
    if (gatedSection) gatedSection.classList.add("open");

    setTimeout(() => { if (gateSuccess) gateSuccess.classList.add("show"); }, 480);
    ...
    setTimeout(() => {
      ...
      if (statsEl) statsEl.scrollIntoView({ block: "start" });
    }, 700);
  });
}
